...
def paginas_webs_function():
    paginas_webs_salvas = {}
    lista_keys = []
    pags = basicos.lerArquivos('paginas webs/lista_paginas/', 'keys_paginas_web', 'txt')
    cont = 0
    for c in pags:
        ...
        if resp == 1:
            ...
            if pagExcluir == -1:
                ...
                for value in pws.values():
                    listaV.append(value)
                newPaginas_webs = { key: pws[key] for key in pws if pws[key] != listaV[pagExcluir] }
                npw = ''
                for key in newPaginas_webs.keys():
                    npw += key + "\n"
                basicos.criaArquivos('paginas webs/lista_paginas/', 'keys_paginas_web', 'txt', f'{npw}', 'w')
        else:
            break
